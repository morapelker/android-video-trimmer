...
private fun setDataSource(mp: MediaMetadataRetriever, path: String, context: Context) {
  MediaPlayerUtils.setMediaPlayerDataSource(context, mp, path)
}

fun extractVideoLength(videoPath: String, context: Context): Long {
  val retriever = try {
    MediaMetadataRetriever()
      .apply { setDataSource(this, videoPath, context) }
  } catch (e: Exception) {
    return 0L
  }
